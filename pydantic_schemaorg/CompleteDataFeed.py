...
class CompleteDataFeed(SchemaOrgBase):
    ...
    Model depth: 5
    """
    type_: str = Field(default="CompleteDataFeed", alias='@type')
    
    dataFeedElement: Union[List[str], str] = Field(
        default=None,
        description="An item within a data feed. Data feeds may have many elements.",union_mode="smart"
    )
    ...
    includedDataCatalog: Union[List[str], str] = Field(
        default=None,
        description="A data catalog which contains this dataset (this property was previously 'catalog',"
     "preferred name is now 'includedInDataCatalog').",union_mode="smart"
    )
    
    variablesMeasured: Union[List[str], str] = Field(
        default=None,
     ...
     "described as text or as pairs of identifier and description using PropertyValue.",union_mode="smart"
    )
    
    catalog: Union[List[str], str] = Field(
        default=None,
        description="A data catalog which contains this dataset.",union_mode="smart"
    )
    
    distribution: Union[List[str], str] = Field(
        default=None,
     ...
     "the entire dataset, for example.",union_mode="smart"
    )
    
    datasetTimeInterval: Union[List[str], str] = Field(
        default=None,
        description="The range of temporal applicability of a dataset, e.g. for a 2011 census dataset, the"
     "year 2011 (in ISO 8601 time interval format).",union_mode="smart"
    )
    
    includedInDataCatalog: Union[List[str], str] = Field(
        default=None,
        description="A data catalog which contains this dataset.",union_mode="smart"
    )
...
if TYPE_CHECKING:
    from pydantic_schemaorg import Text, Thing, DataFeedItem
    from pydantic_schemaorg import DataDownload, StatisticalVariable, datetime, Property, DateTime, PropertyValue, DataCatalog, Text, URL, DefinedTerm, MeasurementMethodEnum, AnyUrl
