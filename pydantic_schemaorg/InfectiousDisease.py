...
class InfectiousDisease(SchemaOrgBase):
    ...
    infectiousAgent: Union[List[str], str] = Field(
        default=None,
        description="The actual infectious agent, such as a specific bacterium.",union_mode="smart"
    )
    
    infectiousAgentClass: Union[List[str], str] = Field(
        default=None,
        description="The class of infectious agent (bacteria, prion, etc.) that causes the disease.",union_mode="smart"
    )
    
    transmissionMethod: Union[List[str], str] = Field(
        default=None,
        description="How the disease spreads, either as a route or vector, for example 'direct contact', 'Aedes"
     "aegypti', etc.",union_mode="smart"
    )
...
if TYPE_CHECKING:
    from pydantic_schemaorg import Text, InfectiousAgentClass
