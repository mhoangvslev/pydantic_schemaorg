...
class MedicalConditionStage(SchemaOrgBase):
    """A stage of a medical condition, such as 'Stage IIIa'.

    See: https://schema.org/MedicalConditionStage
    Model depth: 4
    """
    type_: str = Field(default="MedicalConditionStage", alias='@type')
    
    stageAsNumber: Union[List[str], str] = Field(
        default=None,
        description="The stage represented as a number, e.g. 3.",union_mode="smart"
    )
    
    subStageSuffix: Union[List[str], str] = Field(
        default=None,
        description="The substage, e.g. 'a' for Stage IIIa.",union_mode="smart"
    )
...
if TYPE_CHECKING:
    from pydantic_schemaorg import Text, StrictInt, StrictFloat, Number
