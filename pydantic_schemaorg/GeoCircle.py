...
class GeoCircle(SchemaOrgBase):
    ...
    geoRadius: Union[List[str], str] = Field(
        default=None,
        description="Indicates the approximate radius of a GeoCircle (metres unless indicated otherwise"
     "via Distance notation).",union_mode="smart"
    )
    
    geoMidpoint: Union[List[str], str] = Field(
        default=None,
        description="Indicates the GeoCoordinates at the centre of a GeoShape, e.g. GeoCircle.",union_mode="smart"
    )
...
if TYPE_CHECKING:
    from pydantic_schemaorg import Text, Distance, GeoCoordinates, Number, StrictInt, StrictFloat
