...
class MediaReview(SchemaOrgBase):
    ...
    originalMediaContextDescription: Union[List[str], str] = Field(
        default=None,
        description="Describes, in a [[MediaReview]] when dealing with [[DecontextualizedContent]],"
     "background information that can contribute to better interpretation of the [[MediaObject]].",union_mode="smart"
    )
    
    originalMediaLink: Union[List[str], str] = Field(
        default=None,
        description="Link to the page containing an original version of the content, or directly to an online"
     "copy of the original [[MediaObject]] content, e.g. video file.",union_mode="smart"
    )
    
    mediaAuthenticityCategory: Union[List[str], str] = Field(
        default=None,
        description="Indicates a MediaManipulationRatingEnumeration classification of a media object"
     "(in the context of how it was published or shared).",union_mode="smart"
    )
...
if TYPE_CHECKING:
    from pydantic_schemaorg import Text, MediaObject, URL, MediaManipulationRatingEnumeration, AnyUrl, WebPage
