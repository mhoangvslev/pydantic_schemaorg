...
class DeliveryEvent(SchemaOrgBase):
    ...
    availableThrough: Union[List[str], str] = Field(
        default=None,
        description="After this date, the item will no longer be available for pickup.",union_mode="smart"
    )
    ...
    availableFrom: Union[List[str], str] = Field(
        default=None,
        description="When the item is available for pickup from the store, locker, etc.",union_mode="smart"
    )
    
    accessCode: Union[List[str], str] = Field(
        default=None,
        description="Password, PIN, or access code needed for delivery (e.g. from a locker).",union_mode="smart"
    )
...
if TYPE_CHECKING:
    from pydantic_schemaorg import Text, datetime, DeliveryMethod, DateTime
