...
class ProductModel(SchemaOrgBase):
    ...
    predecessorOf: Union[List[str], str] = Field(
        default=None,
        description="A pointer from a previous, often discontinued variant of the product to its newer variant.",union_mode="smart"
    )
    
    successorOf: Union[List[str], str] = Field(
        default=None,
        description="A pointer from a newer variant of a product to its previous, often discontinued predecessor.",union_mode="smart"
    )
...
if TYPE_CHECKING:
    from pydantic_schemaorg import ProductGroup, ProductModel
