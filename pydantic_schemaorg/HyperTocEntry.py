...
class HyperTocEntry(SchemaOrgBase):
    ...
    utterances: Union[List[str], str] = Field(
        default=None,
        description="Text of an utterances (spoken words, lyrics etc.) that occurs at a certain section of"
     "a media object, represented as a [[HyperTocEntry]].",union_mode="smart"
    )
    
    tocContinuation: Union[List[str], str] = Field(
        default=None,
        description="A [[HyperTocEntry]] can have a [[tocContinuation]] indicated, which is another [[HyperTocEntry]]"
     "that would be the default next item to play or render.",union_mode="smart"
    )
...
if TYPE_CHECKING:
    from pydantic_schemaorg import Text, MediaObject, HyperTocEntry
