...
class PlayGameAction(SchemaOrgBase):
    """The act of playing a video game.

    See: https://schema.org/PlayGameAction
    Model depth: 4
    """
    type_: str = Field(default="PlayGameAction", alias='@type')
    
    gameAvailabilityType: Union[List[str], str] = Field(
        default=None,
        description="Indicates the availability type of the game content associated with this action, such"
     "as whether it is a full version or a demo.",union_mode="smart"
    )
    
    actionAccessibilityRequirement: Union[List[str], str] = Field(
        default=None,
...
if TYPE_CHECKING:
    from pydantic_schemaorg import Text, GameAvailabilityEnumeration
    from pydantic_schemaorg import Offer, ActionAccessSpecification
